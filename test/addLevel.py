...
def main():
    ...
    # Variável para armazenar a experiência
    xp = 0
    ...
    def remove_colliding_enemies(enemies):
        ...
        for enemy_pos in enemies:
            ...
            if not character_rect.colliderect(enemy_rect):
                ...
                increment_xp(random.randint(2, 10))

        return non_colliding_enemies
    ...
    # Função para desenhar a barra de experiência na tela e controlar o nível do personagem
    def draw_xp_bar(xp):
        bar_width = 200
        bar_height = 20
    ...
    # Função para incrementar a experiência
    def increment_xp(gain_xp):
        nonlocal xp
        xp += gain_xp
    ...
    while running:
        ...
        enemies = remove_colliding_enemies(enemies)
        ...
        if time.time() - last_enemy_add_time > 5:  # Adiciona novos inimigos a cada 5 segundos
            enemies = add_new_enemies(enemies, random.randint(10, 20))  # Adiciona novos inimigos aleatórios entre 10 e 20
